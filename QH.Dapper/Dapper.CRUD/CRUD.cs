    ...
    public static partial class CRUD
    {
        ...
        private static void BuildUpdateSet<T>(T entityToUpdate, StringBuilder masterSb)
        {
            ...
            for (var i = 0; i < nonIdProps.Length; i++)
            {
                ...
                var _value = property.GetValue(entityToUpdate, null);
                if (_value == null)
                    continue;
                key += _value;
                str_sb += string.Format("{0} = @{1}", GetColumnName(property), property.Name);
                if (i < nonIdProps.Length - 1)
                    str_sb += ",";
            }
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_BuildUpdateSet{key}", sb =>
            {
                str_sb = str_sb.TrimEnd(',');
                sb.Append(str_sb);
            });
        }
        ...
        private static bool IsNotMapped(PropertyInfo pi)
        {
            ...
            if (attributes.Length > 0)
            {
                dynamic write = attributes.FirstOrDefault(x => x.GetType().Name == typeof(NotMappedAttribute).Name);
                if (write != null)
                {
                    return write.NotMapped;
                }
            }
            return false;
        }
        ...
        private static IEnumerable<PropertyInfo> GetUpdateableProperties<T>(T entity)
        {
            ...
            //remove ones with ID
            updateableProperties = updateableProperties.Where(p => !p.Name.ToUpper().Equals("ID", StringComparison.OrdinalIgnoreCase));
